...
public:
    ...
    wxString m_sfieldData;
};

WX_DECLARE_OBJARRAY(FieldData, ArrayFieldData); //Generic used to get field and data information from a query
WX_DECLARE_OBJARRAY(TableFieldItem, ArrayTableFields);

class FieldRecord
{
public:
    ArrayFieldData aFieldData;
    FieldRecord(){
    };
    //Search for the data we want and return it.
    wxString GetData(wxString FieldName){
        for(int index=0;index<aFieldData.GetCount();index++){

           // FieldData *pFieldData= &aFieldData[index];

            if(aFieldData[index].m_sfieldName==FieldName)
                return aFieldData[index].m_sfieldData;
        }
        return "";
    };
};
...
WX_DECLARE_OBJARRAY(FieldRecord, ArrayFieldRecord);
...
public:
    ...
    static wxString EscapeQuery(wxString QueryString);// Prepares a query to get used in an update or insert statement.
    ...
    //These functions work on tables that contain table_data field, like the usr_controls table
    static bool LoadTableData(wxString sDatabase, wxString sTableName, wxString sTableId, wxString KeyName, wxString &sData, wxString &entireDataString, int &startPosOfData);
    static bool LoadTableData(wxString sDatabase, wxString sTableName, wxString sTableId, wxString KeyName, wxString &sData,wxString sEntireTableDataString="");
    static void SaveTableData(const wxString& sDatabase, const wxString& sTableName, const wxString& sTableId, const wxString& KeyName, wxString data);
    static wxString GetTableDataString(wxString sDatabase, wxString sTableName, wxString sTableId);

    //DATABASE UTILITIES
    static wxString GetHighestId(wxString sTableName);
    ...
    static void GetRecordIDFromTableWhereFieldEquals(wxString sDatabase, wxArrayString &sArray, wxString sTableName, wxString sFieldName, wxString value);
    ...
    static bool LoadStringArrayWithFieldNamesFromQuery(wxArrayString &aFields, wxString sQuery);
    static bool LoadStringArrayWithSingleFieldRecordsFromTableWhereFieldEquals(wxString sDatabase, wxArrayString &sArray, wxString sTableName, wxString sFieldToGet, wxString sFieldName, wxString value);
    ...
    static bool ExecuteQuery(const wxString& sDatabase , const wxString& QueryString,bool bDisplayErrorMessage=true);
    static bool ExecuteQuery(const wxString& QueryString,bool bDisplayErrorMessage=false);
